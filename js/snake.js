class Snake {
    ...
    updateSnake() {
        ...
        if(newHead.x < 0) this.gameOver();
        else if(newHead.y < 0) this.gameOver();
        else if(newHead.x + this.blockSize.w > this.canvas.width) this.gameOver();
        else if(newHead.y + this.blockSize.h > this.canvas.height) this.gameOver();
     ...
     */
    gameOver() {
        let scores = this.scoreEl.innerText;
    ...
    events() {
        ...
        this.canvas.addEventListener('keyup', function(e) {
            if((e.key == 'w' || e.key == 'ArrowUp') && _this.snakeDirection !== 'down' && _this.snakeDirection !== 'up') {
                _this.snakeDy = -_this.blockSize.h;
                _this.snakeDx = 0;
                _this.snakeDirection = 'up'
                _this.update(_this.timestamp.valueOf())
            }else if((e.key == 's' || e.key == 'ArrowDown') && _this.snakeDirection !== 'up' && _this.snakeDirection !== 'down') {
                _this.snakeDy = _this.blockSize.h;
                _this.snakeDx = 0;
                _this.snakeDirection = 'down'
                _this.update(_this.timestamp.valueOf())
            }else if((e.key == 'd' || e.key == 'ArrowRight') && _this.snakeDirection !== 'left' && _this.snakeDirection !== 'right') {
                _this.snakeDy = 0;
                _this.snakeDx = _this.blockSize.w;
                _this.snakeDirection = 'right'
                _this.update(_this.timestamp.valueOf())
            }else if((e.key == 'a' || e.key == 'ArrowLeft') && _this.snakeDirection !== 'right' && _this.snakeDirection !== 'left') {
                _this.snakeDy = 0;
                _this.snakeDx = -_this.blockSize.w;
